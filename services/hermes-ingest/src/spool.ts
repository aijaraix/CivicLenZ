...
export class DurableIntakeSpool {
  ...
  async accept(envelope: ResearchIngestEnvelope, rawBody: Buffer, acknowledgementState: StoredReceipt["acknowledgement_state"], canary?: CanaryAuthorization): Promise<SpoolAcceptResult> {
    await this.initialize();
    ...
    return this.withJobLock(producerId, jobId, async () => {
      ...
      if (existingIndex) {
        if (canary) return { kind: "canary_rejected" };
      ...
      const receiptId = canary?.authorization_id ?? randomUUID();
      ...
      if (canary) {
        if (!validAuthorization(canary)) return { kind: "canary_rejected" };
