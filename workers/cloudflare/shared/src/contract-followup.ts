...
export async function runValidationFollowup(input: {
  ...
  if (m?.schemaVersion !== 'hermes.validation-followup.v1' || !uuid(m.job_id)
  ...
  if (!canonical(j) || j.job_type !== 'contract_scope_research' || p.scope_key !== 'current_occupant'
      ...
      || link.allowance !== 'validation-followup-initial-v1' || link.scope !== 'current_occupant'
      ...
      || route.version !== FOLLOWUP_VERSION || route.capability !== 'current_occupant_context_research'
      ...
      || route.source_url !== config.baseUrl || route.retrieval_url !== 'https://www.flgov.com/eog/'
  ...
  const lineage = { job_id: j.job_id, research_need_id: j.research_need_id, research_work_identity: j.dedupe_key,
    receipt_id: link.receipt_id, receipt_job_id: link.receipt_job_id, evidence_need_id: link.evidence_need_id,
    ...
    route, tool: 'http.ts:fetchDocument', parser_key: 'official-profile-discovery', parser_version: FOLLOWUP_VERSION };
  await db('worker_runs', 'POST', { worker_run_id: runId, job_id: j.job_id,
    worker_key: 'hermes.cloudflare.validation_followup', runtime: 'cloudflare', deployment_id: input.deploymentId,
  ...
  try {
    ...
    const [field] = await db(`research_contract_fields?research_contract_id=eq.${p.contract_id}&field_key=eq.current_occupant`);
    if (!claim || claim.subject_type !== 'seat' || claim.subject_id !== j.target_id || claim.seat_id !== j.target_id
        ...
        || field?.verification_requirement !== 'official_source' || field?.sensitivity_rule !== 'publication_eligible_claims_only'
    ...
    await db('raw_retrievals', 'POST', { retrieval_id: retrievalId, source_id: route.source_id, job_id: j.job_id,
      ...
      parser_key: 'official-profile-discovery', parser_version: FOLLOWUP_VERSION, metadata: lineage });
    // Preserve source bytes before parsing; parser failure never erases retrieval.
    const parsed = await dispatchSourceAdapter({ sourceKey: route.source_key, bytes: saved,
    ...
    const html = new TextDecoder('utf-8', { fatal: true }).decode(saved), pos = html.indexOf(holder.displayName);
    if (pos < 0) throw Error('followup_locator_missing');
    const start = Math.max(0, pos - 120), excerpt = html.slice(start, pos + holder.displayName.length + 120);
    ...
    const result = { ...lineage, retrieval_id: retrievalId, evidence_id: evidenceId, sha256: digest,
      ...
      source_suitability: { state: 'OFFICIAL_EXECUTIVE_PROFILE_CANDIDATE', source_id: route.source_id,
        reason: 'Registered official office source; metadata parser alone does not certify current-office page role' },
      ...
      schema_certified: false, parser_certification: { state: 'NOT_CERTIFIED', parser_version: FOLLOWUP_VERSION,
        reason: 'Single metadata-derived holder and exact locator are insufficient; no global source certification' },
    ...
    await db('validation_runs', 'POST', { validation_run_id: validationId, subject_type: 'seat', subject_id: j.target_id,
      seat_id: j.target_id, validator_key: FOLLOWUP_VERSION, status: 'NEEDS_FURTHER_VALIDATION',
    ...
    const code = error instanceof Error && /^[a-z0-9_]+$/.test(error.message) ? error.message : 'followup_execution_failed';
    await db(`worker_runs?worker_run_id=eq.${runId}&status=eq.started`, 'PATCH', { status: 'failed', completed_at: completeTime(),
      error_class: code, error_message: 'Follow-up failed; preserve artifacts and consumed allowance', metadata: { ...lineage, retryable: false } });
    throw error;
  }
}
